...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/07/16 14:05:16 by akurilen          #+#    #+#             */
/*   Updated: 2018/07/16 14:05:18 by akurilen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	spider_work(t_win *win)
{
	...
	i = 0;
	while (++i <= 8)
		pthread_create(&t[i - 1], NULL, cord_vect_spider, &map[i - 1]);
	while (i--)
...
void	*cord_vect_spider(void *data)
{
	...
	while (++map->x < 1024)
	{
		...
		{
			map->pr = 1.5 * (map->x - 512) / (map->zoom * 256) + map->move_x;
			map->pi = (map->y - 512) / (map->zoom * 256) + map->move_y;
			map->new_i = 0;
			map->new_r = 0;
			iter = -1;
			while (++iter < map->max_iter &&
				(map->new_r * map->new_r * map->new_r +
					map->new_i * map->new_i * map->new_i) < 4)
				calc_spider(map);
			map_color(iter, map);
...
void	calc_spider(t_win *map)
{
	...
	map->pr = map->new_r + map->pr / 2;
	map->pi = map->new_i + map->pi / 2;
}
