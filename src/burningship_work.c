...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/07/21 17:49:15 by akurilen          #+#    #+#             */
/*   Updated: 2018/07/21 17:49:18 by akurilen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	burningship_work(t_win *win)
{
	...
	i = 0;
	while (++i <= 8)
		pthread_create(&t[i - 1], NULL, cord_vect_burningship, &map[i - 1]);
	while (i--)
...
void	calc_burningship(t_win *map)
{
	map->old_r = map->new_r;
	map->old_i = map->new_i;
	map->new_r = map->old_r * map->old_r - map->old_i * map->old_i + map->pr;
	map->new_i = fabs(2 * map->old_r * map->old_i) + map->pi;
}
